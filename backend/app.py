...
def initialize_firebase():
    ...
    global firebase_initialized, db, firebase_error
...
# ── Initialize Firebase at module load (works under gunicorn too) ──────────────
initialize_firebase()
...
def generate_recommendation(probability, data):
    ...
    if risk == "Low":
        ...
        return " | ".join(recommendations) if recommendations else \
            "Increase monitoring frequency and extend withdrawal period"

    else:  # High risk
        return ("⚠️ HIGH RISK: Do NOT sell animal. Consult veterinarian immediately. "
                "Extend withdrawal period by minimum 10-14 days and retest.")
...
def home():
    ...
    return jsonify({
        ...
        "endpoints": {
            ...
            "/api/tts": "POST - Text-to-speech (Tamil / English)"
        },
        "version": "1.1.0"
    }), 200
...
def health_check():
    ...
    return jsonify({
        ...
        "firebase_connected": firebase_initialized,   # use own flag, not private _apps
        "firebase_error": firebase_error,             # surfaces the real error if any
        "timestamp": datetime.now().isoformat()
    }), 200
...
def predict():
    ...
    try:
        ...
        required_fields = ['animal_type', 'antibiotic_type', 'dosage_mg',
                           'duration_days', 'days_before_sale']
        ...
        print(f"[SUCCESS] Risk: {prediction['risk_level']}, "
              f"Probability: {prediction['violation_probability']}")
...
def classify_intent(message):
    ...
    intents = {
        ...
        'farm_overview': ['my animals', 'என் விலங்குகள்', 'total', 'எத்தனை',
                          'how many', 'list', 'all animals', 'அனைத்து விலங்குகள்',
                          'பட்டியல்', 'list all']
    }
    ...
    try:
        ...
        for t in treatments:
            ...
            if t_data.get('injectionDate'):
                injection_date = t_data['injectionDate']
                if hasattr(injection_date, 'replace'):
                    injection_date = injection_date.replace(tzinfo=None)
        ...
        result = (
            ...
            f"Species: {animal.get('speciesDisplay', 'Unknown')}\n"
        ...
        if active_treatment:
            result += (
                f"\nACTIVE TREATMENT:\n"
                f"Medicine: {active_treatment['medicine']}\n"
                f"Dosage: {active_treatment['dosage']}\n"
                f"Injection Date: {active_treatment['injection_date']}\n"
                f"Withdrawal Ends: {active_treatment['withdrawal_end']}\n"
                f"Days Remaining: {active_treatment['days_remaining']}\n\n"
                f"WARNING: Do NOT sell this animal yet."
            )
        else:
...
def check_withdrawal_safety(farmer_id, animal_id):
    ...
    try:
        ...
        if not treatments:
            return (f"{animal_id}: No treatment records found. "
                    f"Likely safe to sell, but verify with veterinarian.")
        ...
        for t in treatments:
            ...
            if t_data.get('injectionDate'):
                ...
                if withdrawal_end > datetime.now():
                    days_remaining = (withdrawal_end - datetime.now()).days
                    return (
                        f"⚠️ WARNING - {animal_id} NOT SAFE TO SELL\n\n"
                        f"Withdrawal Period: ACTIVE\n"
                        f"Medicine: {t_data.get('antibioticName', 'Unknown')}\n"
                        f"Withdrawal Ends: {withdrawal_end.strftime('%Y-%m-%d')}\n"
                        f"Days Remaining: {days_remaining}\n\n"
                        f"Do NOT sell this animal until withdrawal period completes."
                    )

        return (
            f"✅ {animal_id} - SAFE TO SELL\n\n"
            f"Withdrawal Status: Completed\n"
            f"No active treatments found.\n\n"
            f"This animal appears safe for sale. Consider generating a certificate."
        )
    ...
    try:
        ...
        for idx, t in enumerate(treatments, 1):
            ...
            result += (
                f"{idx}. Date: {date_str}\n"
                f"   Medicine: {t_data.get('antibioticName', 'Unknown')}\n"
                f"   Dosage: {t_data.get('dosage', 'N/A')}\n"
                f"   Withdrawal Days: {t_data.get('withdrawalDays', 'N/A')}\n"
                f"   Purpose: {t_data.get('purpose', 'N/A')}\n\n"
            )
...
def get_mrl_risk_prediction(farmer_id, animal_id):
    ...
    try:
        ...
        dosage_raw = latest_treatment.get('dosage', '0')
        try:
            dosage_mg = float(str(dosage_raw).split()[0])
        ...
        return (
            f"MRL RISK PREDICTION - {animal_id}\n\n"
            f"Risk Level: {prediction['risk_level']}\n"
            f"Violation Probability: {prediction['violation_probability']}%\n\n"
...
def openrouter_chat():
    ...
    try:
        ...
        context_data = None
        if farmer_id and intent in ['animal_status', 'withdrawal_check', 'treatment_history',
                                     'risk_prediction', 'farm_overview']:
            context_data = handle_database_query(farmer_id, intent, entity)
            print(f"CONTEXT DATA: {context_data}")

            if context_data and intent != 'general_question':
                user_message = (
                    f"Format this farm data in a clear, friendly way for the farmer:\n\n"
        ...
        if has_tamil or language == "tamil":
            system_prompt = (
                "You are VetFarm AI Assistant, a smart veterinary and farm management chatbot.\n\n"
...
if __name__ == '__main__':
    print("\n" + "#" * 60)
    print("#  Antibiotic Misuse & MRL Risk Prediction API")
    print("#" * 60)
    ...
    if firebase_initialized:
        ...
        print(f"✗ Firebase Status: NOT CONNECTED — {firebase_error}")
